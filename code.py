...
def func(i):
    ...
    ls.append([price, book_title['title'],linkk['href'],"pushtkosh","no rating available"])
...
def func1(i):
    ...
    price=price_text_to_string(price.text)
    ls.append([price, book_title['title'],mn_url+linkk['href'],"flipkart",rating.text])
...
def func(i):
    ...
    ls.append([price, book_title['title'], linkk['href'], "pushtkosh", img_src['src'],"no rating available"])
...
def func1(i):
    ...
    price = price_text_to_string(price.text)
    ls.append([price, book_title['title'], mn_urlflp + linkk['href'], "flipkart", img_src['src'],rat])
...
books_set_lis_bookswagon = soup_bookswagon.find_all('div', {'class': book_classes_bookswagon})
...
def func2(i):
    ...
    linkk = books_set_lis_bookswagon[i].find('div', {'class': bookswagon_name_link_class})
    ...
    # print("link :", mn_url+linkk['href'] )
    price = price_text_to_string(price.text)
    ls.append([price, book_title.text, book_title['href'], "bookswagon", img_src['src'],"no rating available"])
...
def func3(i):
    ...
    book_title = books_set_lis_sapna[i].find('h2', {'class': sapna_name_link_class})
    ...
    price = price_text_to_string(price.text)
    ls.append([price, book_title.text, url_sapna_main + linkk['href'], "sapna", img_src['src'],"no rating available"])
...
book_set_list_amazon=soup_amazon.find_all('div',{'class': book_set_Class_amazon})
...
def func5(i):
    ...
    book_price=book_set_list_amazon[i].find('span',{'class' : book_price_class})
    if(book_price==None):
        return
    ...
    book_title=book_set_list_amazon[i].find('span',{'class':book_title_class})
    ...
    book_rating_div=book_set_list_amazon[i].find('div',{'class':book_rating_class})
    book_rating=book_rating_div.find('span')
    ls.append([book_rate,book_title.text,mn_url_amazon+link['href'],"amazon",img_link['src'],book_rating['aria-label']])
